...
impl Pawlicki<'_> {
    ...
    pub fn from_file<'a>(location1: &'a str, location2: &'a str, fb: bool) -> Result<Pawlicki<'a>, Box<dyn Error>> {
    ...
    pub fn fallback(&self, model: Option<&WordVector>, input: &str) -> String {  //Choose a fallback based on sentences additive cosine similarity
        ...
        for fallback in &self.fallbacks{
            ...
            for input_word in &input_split {
                for fallback_word in &fallback_split {
                    let vec1 = model.unwrap().get_vector(input_word.as_ref());
                    let vec2 = model.unwrap().get_vector(fallback_word.as_ref());
                    match (vec1, vec2) {
                        (Some(_), Some(_)) => {
                                                num_words += 1.0;
    ...
    pub fn respond(&mut self, model: &Option<WordVector>, input: &str) -> String {
        ...
        if let Some(phrase) = active_phrase {
            ...
            response = mem;
                ...
                if self.use_fb {
                    response = self.fallback(model.as_ref(), input);
                } else {
                    response = format!("Why don't you go checkout our website or ask the undergrad coordinator?");
                }
            }
        } else if let Some(mem) = self.memory.pop_front() {
            info!("Using memory");
...
println!("cap: {:?}", cap);
                        ...
                        if r.lookup {
                            println!("lookup true");
                            data = self.get_query(&r.decomposition_rule, &r.lookup_rule, &cap);
                        }
...
println!("assemble: {:?}", assem);
                            ...
                            response = assemble(&assem, &cap, &self.script.reflections, &data);
    ...
    fn get_query<'t>(&mut self, dr: &str, lr: &str, captures: &Captures<'t>) -> Data {
...
println!("params: {:?}", params);
        let ret = self.database.query_executor(lr, &params);
        ret
    }

    fn get_reassembly(&mut self, id: &str, rules: &[String], data: &Data) -> Option<String> {
...
fn assemble(rule: &str, captures: &Captures<'_>, reflections: &[Reflection], data: &Data) -> Option<String> {
    println!("{:?}", data);
